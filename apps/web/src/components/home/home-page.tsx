...
function HeroSection({ featuredTools }: { featuredTools: FeaturedTool[] }) {
  const { dict } = useI18n();
  ...
  const featured =
    featuredTools.length > 0
      ? featuredTools.slice(0, 4).map((t) => ({ id: t.slug, name: t.name }))
      : dict.home.featuredTools.slice(0, 4);
...
export function HomePage({ featuredTools }: { featuredTools: FeaturedTool[] }) {
  const { dict } = useI18n();
