...
const RELATED_SLUGS = ["image-converter", "image-cropper", "image-compressor"] as const;
...
const LONG_DESCRIPTION =
  "Our Image Resizer uses the Canvas API to resize images to exact pixel dimensions in your browser. Set the width and height, lock the aspect ratio for a perfect fit, and download every resized image — all locally, with no uploads.";
...
const FAQ = [
  {
    ...
    answer: "Each image is drawn onto a canvas at the target dimensions and re-encoded. Quality stays high, and the aspect lock keeps proportions intact.",
  },
  {
    question: "Where are my images processed?",
    answer: "100% in your browser. Images are never uploaded, making the tool fast and private.",
  },
  {
    question: "What formats are supported?",
    answer: "PNG, JPEG, and WebP inputs, and you can choose any of those as the output format.",
  },
];

const ARTICLE = {
  title: "The Right Size, Every Time",
  content:
    "From social media thumbnails to blog hero images, every platform has preferred dimensions. Manually resizing one image is slow; doing dozens is painful. Batch resizing with an aspect lock gives you consistent, platform-ready images in seconds — all without leaving your browser.",
};
...
export default function ImageResizerPage() {
  ...
  const meta = t.meta["image-resizer"];
  const ir = t.imageResizer;
  ...
  const [results, setResults] = useState<Map<string, ResizeResult>>(new Map());
  ...
  const updateWidth = useCallback((value: string) => {
    setWidth(value);
    if (locked && images.length > 0) {
      const first = images[0];
      if (first) {
        const w = parseInt(value, 10);
        if (!Number.isNaN(w) && w > 0) {
          setHeight(String(Math.round((w * first.height) / first.width)));
        }
      }
    }
  }, [locked, images]);
    ...
    if (locked && images.length > 0) {
      ...
      if (first) {
        ...
        if (!Number.isNaN(h) && h > 0) {
          setWidth(String(Math.round((h * first.width) / first.height)));
        }
      }
    }
  }, [locked, images]);

  const resizeSingle = useCallback(
    (image: ImageFile): Promise<ResizeResult> => {
      return new Promise((resolve) => {
        const w = Math.max(1, Math.min(8000, parseInt(width, 10) || 800));
        const h = locked
          ? Math.max(1, Math.round((w * image.height) / image.width))
          : Math.max(1, Math.min(8000, parseInt(height, 10) || 800));
        const img = new Image();
        img.onload = () => {
          ...
          const outDataUrl = canvas.toDataURL(mimeType, 0.9);
          const outSize = Math.round((outDataUrl.length * 3) / 4);
          resolve({ id: image.id, dataUrl: outDataUrl, size: outSize, width: w, height: h });
        };
        img.src = image.dataUrl;
      });
    },
    [width, height, locked, format]
  );
  ...
  const resizeAll = useCallback(async () => {
    ...
    for (let i = 0; i < images.length; i++) {
      const img = images[i];
      if (!img) continue;
      const result = await resizeSingle(img);
      newResults.set(img.id, result);
      setProgress({ completed: i + 1, total: images.length });
    }
    setResults(newResults);
    setProcessing(false);
  }, [images, results, resizeSingle]);

  const downloadImage = useCallback(
    (result: ResizeResult, name: string) => {
      const link = document.createElement("a");
      link.href = result.dataUrl;
      const ext = format === "png" ? "png" : format === "webp" ? "webp" : "jpg";
      link.download = name.replace(/\.[^.]+$/, "") + `-resized.${ext}`;
      link.click();
    },
    [format]
  );
  ...
  const inputCls =
    "w-full rounded-lg border border-neutral-300 bg-white px-3 py-2 text-sm text-neutral-900 focus:border-brand-500 focus:outline-none focus:ring-2 focus:ring-brand-500 dark:border-neutral-600 dark:bg-neutral-800 dark:text-white";

  const firstImage = images[0];
  const lockedHeight = (() => {
    const w = parseInt(width, 10);
    if (firstImage && !Number.isNaN(w) && w > 0) {
      return String(Math.round((w * firstImage.height) / firstImage.width));
    }
    return "";
  })();
      ...
      <div className="space-y-6">
        ...
        <div className="grid gap-4 sm:grid-cols-3">
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className={labelCls}>{ir.width}</label>
              <input type="number" min={1} max={8000} value={width} onChange={(e) => updateWidth(e.target.value)} className={inputCls} />
            </div>
            <div>
              <label className={labelCls}>{ir.height}</label>
              <input
                type="number"
                min={1}
                max={8000}
                value={locked ? lockedHeight : height}
                disabled={locked}
          ...
          <div>
            <label className={labelCls}>{ir.outputFormat}</label>
          ...
          <div className="flex items-end gap-2">
            ...
            >
              {processing
                ? ir.processing.replace("{done}", String(progress.completed)).replace("{total}", String(progress.total))
                : ir.resizeAll.replace("{count}", String(images.length))}
            </button>
            {results.size > 0 && (
        ...
        {images.length === 0 ? (
          <div className="rounded-xl border border-neutral-200 bg-neutral-50 p-12 text-center dark:border-neutral-700 dark:bg-neutral-800">
            <p className="text-neutral-400">{ir.noImagesUploaded}</p>
          </div>
        ) : (
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    {result && (
                      <div className="mt-1 flex items-center gap-2 text-xs text-neutral-400">
                        <span>{ir.newSize}: {result.width}×{result.height} | {formatFileSize(result.size)}</span>
                      </div>
                    )}
                  </div>
