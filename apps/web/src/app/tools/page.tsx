...
function ToolsBrowser() {
  ...
  const filtered = React.useMemo(() => {
    ...
    return ALL_TOOL_SLUGS.filter((slug) => {
      if (category && !CATEGORY_ORDER[category].includes(slug)) return false;
      if (!q) return true;
      const meta = dict.tools.meta[slug as keyof typeof dict.tools.meta];
      const text = `${meta?.name ?? ""} ${meta?.short ?? ""} ${meta?.description ?? ""} ${slug}`.toLowerCase();
    ...
    <div className="min-h-screen bg-white dark:bg-neutral-950">
      ...
      <section className="py-12">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          ...
          >
            {filtered.map((slug, i) => (
              <ToolCard key={slug} slug={slug} delay={Math.min(i * 0.03, 0.2)} />
            ))}
          </motion.div>
